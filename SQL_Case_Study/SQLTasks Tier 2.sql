...
THEN 'expensive'
ELSE 'cheap'
END AS maintenance_cost_description
FROM Facilities
...
WHERE starttime LIKE '2012-09-14%'
  AND (	
	(memid =0 AND f.guestcost >30) 
   OR (memid !=0 AND f.membercost >30)
    )
ORDER BY starttime, cost DESC
...
SELECT * 
  FROM 
	(SELECT b.starttime, f.name AS facility,
			CASE WHEN m.memid =0 AND f.guestcost >30 THEN f.guestcost
			WHEN m.memid !=0 AND f.membercost >30 THEN f.membercost
			END AS cost, CONCAT( m.firstname, ' ', m.surname ) AS member_name
	  FROM Bookings AS b
		INNER JOIN 
